...
/* FKinectBoneRotatorConstraints structure
	This structure is used to define constrains for each bone rotator
	The idea is to define a min/max rotator and a flag to enable it
*/
USTRUCT(BlueprintType)
struct FKinectBoneRotatorConstraints
{
...
public:
	...
	FKinectBoneRotatorConstraints()
	{

	}

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Kinect)
		FRotator MinRotator = FRotator(0);

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Kinect)
		FRotator MaxRotator = FRotator(0);

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Kinect)
		bool bEnabled = false;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Kinect)
		FRotator MaxAbsDelta = FRotator(20);

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Kinect)
		bool bCheckMaxDeltaEnabled = false;
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, EditFixedSize, Category = Kinect)
		TArray<FKinectBoneRotatorConstraints> RotatorAdjustments;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, EditFixedSize, Category = Kinect)
		TArray<bool> debugRotators;
...
private:
	...
	TArray<FRotator> prevKinectBoneRotators;

	inline FRotator RMin(const FRotator & r1, const FRotator & r2)
	{
		FRotator temp;
		temp.Pitch = FMath::Min(r1.Pitch, r2.Pitch);
		temp.Yaw = FMath::Min(r1.Yaw, r2.Yaw);
		temp.Roll = FMath::Min(r1.Roll, r2.Roll);
		return temp;
	}

	inline FRotator RMax(const FRotator & r1, const FRotator & r2)
	{
		FRotator temp;
		temp.Pitch = FMath::Max(r1.Pitch, r2.Pitch);
		temp.Yaw = FMath::Max(r1.Yaw, r2.Yaw);
		temp.Roll = FMath::Max(r1.Roll, r2.Roll);
		return temp;
	}
